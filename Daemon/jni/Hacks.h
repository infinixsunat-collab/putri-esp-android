...
void DrawESP(ESP esp, int screenWidth, int screenHeight) {
   ...
   esp.DrawText(Color::White(), mFPS.c_str(), {120, 120}, screenWidth / 70);
   ...
   if (response.Success){	    
       ...
       int32_t MonsterCount = response.MonsterCount;
       for (int i = 0; i < MonsterCount; i++) {
            MonsterData mons = response.Monsters[i];
            if (mons.position.x < 0 || mons.position.x > screenWidth || mons.position.y < 0 || mons.position.y > screenHeight) {
                continue;
          }
          float IconSize = screenWidth / 11.4;
          Vector2 p_min = {mons.position.x - ((IconSize / 3) / 2), mons.position.y - ((IconSize / 3) / 2)};
          esp.DrawBase64Bitmap(mons.m_ID, IconSize / 3, IconSize / 3, {p_min.x, p_min.y});      
          float max_health = mons.HealthMax;
          float health = mons.Health;
          float a_max = ((M_PI * 2.0f));
          esp.DrawCircularHealth(Color::Red(), health, max_health, mons.position, IconSize / 5.5);
